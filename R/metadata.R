...
read_dfr_citations <- function(filename=NA, ...) {
    ...
    if (grepl("\\.tsv", f, ignore.case=T)) {
        ...
        result <- read.table(f, header=F, skip=1, sep="\t", col.names=cols,
...
#' 
#' Given a dataframe with metadata and (optionally) document id's, return a
#' character vector of citations.
#' 
